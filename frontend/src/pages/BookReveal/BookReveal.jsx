...
// ─── Mock book data ───────────────────────────────────────────────────────────
const mockBook = {
  title: 'The Warmth She Left Behind',
  subtitle: 'A portrait of Nana Rose — her life, her love, and the memories that will never fade.',
  chapters: 6,
  memoryCount: 9,
  readingTime: '24 min',
  coverGradient: 'from-[hsl(350,40%,80%)] via-[hsl(33,30%,88%)] to-[hsl(40,40%,94%)]',
};
    ...
    <div className="relative z-10 flex flex-col items-center max-w-xl w-full">
      ...
      >
        ...
        <div className="relative">
          ...
          {/* Cover */}
          <div
            className={`w-56 md:w-64 aspect-[3/4] rounded-2xl bg-gradient-to-br ${book.coverGradient} shadow-2xl flex flex-col items-center justify-end p-6 overflow-hidden`}
          >
      ...
      >
        {[
          { icon: BookMarked, label: `${book.chapters} Chapters` },
          { icon: Hash, label: `${book.memoryCount} Memories` },
          { icon: Clock, label: `${book.readingTime} read` },
        ].map(({ icon: Icon, label }) => (
          <span
            key={label}
    ...
    <div className="min-h-screen bg-warm-ivory relative overflow-hidden">
      <AnimatePresence mode="wait">
        {phase === 'processing' ? (
          ...
          <BookRevealScreen key="reveal" book={mockBook} navigate={navigate} />
        )}
      </AnimatePresence>
    </div>
  );
